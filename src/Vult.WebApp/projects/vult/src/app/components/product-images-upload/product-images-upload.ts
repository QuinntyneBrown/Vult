...
import { ProductService, SignalRService } from '../../core/services';
...
@Component({
  ...
  templateUrl: './product-images-upload.html',
  styleUrls: ['./product-images-upload.scss']
})
export class ProductImagesUpload implements OnInit, OnDestroy {
  ...
  async uploadPhotos(): Promise<void> {
    ...
    this.productService.uploadPhotos(files).subscribe({
      next: () => {
